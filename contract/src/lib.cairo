...
struct BCITemplateV3 {
    // ── Fuzzy commitment ──────────────────────────────────────
    // C = BCH_encode(K) XOR B_stable_64
    // commitment_hash = HMAC(K, "CommitmentHash:" + hex(C))
    commitment_hash: felt252,

    // ── 64-bit behavioral feature vector (FIX 1) ─────────────
    // Stored as two felt252 values (high 32 bits + low 32 bits)
    binary_high: felt252,          // bits 63..32 of 64-bit binary string
    binary_low:  felt252,          // bits 31..0  of 64-bit binary string
    ...
    // enrollment_response_seed = HMAC(MS, "BCI_RESPONSE_SEED_V3")
    // Stored on-chain. Cannot be reversed to recover MS.
    // Server uses this to verify: HMAC(seed, nonce:agent_id) == response
    enrollment_response_seed: felt252,

    // ms_commitment = HMAC(MS, "BCI_COMMITMENT_V3")
    // Identity binding — proves MS is bound to this specific agent
    ms_commitment: felt252,

    // ms_receipt_verifier = HMAC(MS, "BCI_MS_RECEIPT_V3")
    // Stored to verify agent's MS receipt without knowing MS
...
struct AgentRegistrationV3 {
    ...
    ms_provisioned:      bool,     // FIX 3: confirmed via ms_receipt

    challenge_failure_count:   u32,
    behavioral_failure_count:  u32,
    locked_until:              u64,  // unix timestamp of lockout expiry
...
struct ChallengeIssuance {
    nonce:      felt252,
    expires_at: u64,
    // Instruction to P-Process: compute HMAC(enrollment_response_seed, nonce:agent_id)
    // P-Process never exposes this instruction to Q-Process (LLM)
}
...
#[starknet::interface]
trait IBCIAgentIdentityV1<TContractState> {
    ...
    fn confirm_ms_provisioned(
        ref self: TContractState,
        agent_id:   felt252,
        ms_receipt: felt252,   // agent sends HMAC(MS, "BCI_MS_RECEIPT_V3")
    );
    ...
    fn submit_challenge_response(
        ...
        agent_id:         felt252,
        nonce:            felt252,
        response_hash:    felt252,   // Poseidon(HMAC_response) for on-chain storage
        response_valid:   bool,      // relayer pre-verified using HMAC off-chain
    ) -> AuthResultV3;

    fn update_template_drift(
        ref self: TContractState,
        agent_id:          felt252,
        new_template:      BCITemplateV3,
        drift_sigma_x100:  u64,
        owner_signature:   felt252,   // required if drift > DRIFT_ALERT_SIGMA
    );
    ...
    // ── Views ─────────────────────────────────────────────────
    fn get_agent(self: @TContractState, agent_id: felt252) -> AgentRegistrationV3;
    fn get_template(self: @TContractState, agent_id: felt252) -> BCITemplateV3;
...
#[starknet::contract]
mod BCIAgentIdentityV1 {
    use super::{
        BCITemplateV3, AgentRegistrationV3, ChallengeIssuance,
        AuthResultV3, IBCIAgentIdentityV1, ContractAddress,
        BCI_VERSION, BINARY_STRING_BITS, ENROLLMENT_PERIOD_SECS,
        MIN_OBSERVATIONS, CHALLENGE_EXPIRY_SECS, MAX_FAILURES,
    ...
    #[storage]
    struct Storage {
        agents:          starknet::storage::Map<felt252, AgentRegistrationV3>,
        templates:       starknet::storage::Map<felt252, BCITemplateV3>,
    ...
    #[abi(embed_v0)]
    impl BCIAgentIdentityV1Impl of IBCIAgentIdentityV1<ContractState> {
        ...
        fn request_authorization(
            ...
            if agent.is_revoked {
                return AuthResultV3 { approved: false, conf_score: 0, spending_limit: 0, flag_owner: true, needs_challenge: false, challenge: empty_challenge, reason: 'REVOKED' };
            }
            if !agent.enrollment_complete {
                return AuthResultV3 { approved: false, conf_score: 0, spending_limit: 0, flag_owner: false, needs_challenge: false, challenge: empty_challenge, reason: 'NOT_ENROLLED' };
            }
            if !agent.ms_provisioned {
                return AuthResultV3 { approved: false, conf_score: 0, spending_limit: 0, flag_owner: false, needs_challenge: false, challenge: empty_challenge, reason: 'MS_NOT_PROVISIONED' };
            }

            if timestamp < agent.locked_until {
                return AuthResultV3 { approved: false, conf_score: 0, spending_limit: 0, flag_owner: true, needs_challenge: false, challenge: empty_challenge, reason: 'LOCKED_OUT' };
            }
            ...
            if timestamp > agent.auth_key_expiry {
                return AuthResultV3 { approved: false, conf_score: behavioral_score, spending_limit: 0, flag_owner: true, needs_challenge: false, challenge: empty_challenge, reason: 'AUTH_KEY_EXPIRED' };
            }
            ...
            // All gates passed — issue challenge
        ...
        fn submit_challenge_response(
            ...
            // Clear challenge state regardless
            agent.challenge_active = false;
            agent.active_nonce     = 0;

            if !response_valid {
                // FIX 2: Track failures, apply lockout
                agent.challenge_failure_count += 1;
            ...
            // Challenge passed — reset failure count, execute payment
        ...
        fn update_template_drift(
            ...
            if drift_sigma_x100 > DRIFT_ALERT_SIGMA_X100 {
                // Verify owner_signature = Poseidon(agent_id, drift_sigma_x100, timestamp/86400)
            ...
            t.commitment_hash           = old_template.commitment_hash;
            t.binary_high               = old_template.binary_high;
            t.binary_low                = old_template.binary_low;
            t.enrollment_response_seed  = old_template.enrollment_response_seed;
            t.ms_commitment             = old_template.ms_commitment;
            t.ms_receipt_verifier       = old_template.ms_receipt_verifier;
            t.enrollment_timestamp      = old_template.enrollment_timestamp;
